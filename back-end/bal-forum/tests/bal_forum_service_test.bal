...
isolated http:Client forumClient = check new (forumServiceUrl);
...
isolated function mockInitDbClient() returns mysql:Client|error {
...
isolated function mockInitSentimentClient() returns http:Client|error {
...
//     http:Response response;
//     lock {
//     response = check forumClient->/users/[userId]/posts.post(newPost, schedule = schedule);
//     }
...
//     string userId = uuid:createType1AsString();
//     test:prepare(forumDBClient).when("queryRow").thenReturn(userId);
//     NewForumPost & readonly newPost = {
//         title: "Ballerina Websub",
//         description: "Ballerina supports websub",
//         timestamp: "2023-12-03T10:15:30.00Z"
//     };
//     string & readonly schedule = time:utcToString(time:utcAddSeconds(time:utcNow(), 100));
//     http:Response response;
//     lock {
//     response = check forumClient->/users/[userId]/posts.post(newPost, schedule = schedule);
//     }
...
//     http:Response response;
//     lock {
//     response = check forumClient->/posts/[postId]/likes.post({userId});
//     }
//     test:assertEquals(response.statusCode, http:STATUS_NOT_FOUND, "Status code mismatched");
...
//     io:println("testCommentOnNonExistingPost");
//     string postId = uuid:createType1AsString();
//     string userId = uuid:createType1AsString();
//     NewPostComment & readonly comment = {userId, comment: "This is a comment", timestamp: "2023-12-03T10:15:30.00Z"};
//     sql:Error noPostError = error("No post found");
//     test:prepare(forumDBClient).when("queryRow").thenReturn(noPostError);
//     http:Response response;
//     lock {
//     response = check forumClient->/posts/[postId]/comments.post(comment);
//     }
//     test:assertEquals(response.statusCode, http:STATUS_NOT_FOUND, "Status code mismatched");
//     test:assertEquals(response.getJsonPayload(), {error_message: "Post not found"}, "Response mismatched");
// }
...
function testCommentOnExistingPost() returns error? {
    io:println("testCommentOnExistingPost");
    string postId = uuid:createType1AsString();
    string userId = uuid:createType1AsString();
    NewPostComment & readonly comment = {userId, comment: "This is a comment", timestamp: "2023-12-03T10:15:30.00Z"};
    ForumPostInDB & readonly forumPost = {
        id: postId,
        user_id: userId,
        title: "Ballerina Websub",
        description: "Ballerina supports websub",
...
function testGetNonExistingPost() returns error? {
    io:println("testGetNonExistingPost");
    string postId = uuid:createType1AsString();
    sql:Error noPostError = error("No post found");
    test:prepare(forumDBClient).when("queryRow").thenReturn(noPostError);
    http:Response response;
    lock {
    response = check forumClient->/posts/[postId];
    }
    test:assertEquals(response.statusCode, http:STATUS_NOT_FOUND, "Status code mismatched");
    test:assertEquals(response.getJsonPayload(), {error_message: "Post not found"}, "Response mismatched");
}
...
//     io:println("testGetExistingPost");
//     string postId = uuid:createType1AsString();
//     string userId = uuid:createType1AsString();
//     ForumPostInDB forumPost = {
//         id: postId,
...
//     ForumPost actualPost;
//     lock {
//         actualPost = check forumClient->/posts/[postId];
//     }
...
//     ForumPost[] actualPosts;
//     lock {
//         actualPosts = check forumClient->/posts;
//     }
